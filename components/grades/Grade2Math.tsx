...
const Grade2Math: React.FC = () => {
  ...
  // Ajouter cette fonction pour obtenir l'heure actuelle formatée
  const getCurrentTime = () => {
    const now = new Date()
    const hours = now.getHours()
    const minutes = now.getMinutes()

    // Format 12-hour avec AM/PM
    const ampm = hours >= 12 ? "P.M." : "A.M."
    const hours12 = hours % 12 || 12 // Convertir 0 en 12
...
//       const savedName = localStorage.getItem("math2StudentName")
//       const savedAnswers = localStorage.getItem("math2Answers")
//       const savedStep = localStorage.getItem("math2CurrentStep")
...
//       // Si on est à l'étape des questions et qu'il n'y a pas de réponses sauvegardées pour les questions d'horloge
//       if (currentStep === 1) {
//         const parsedAnswers = savedAnswers ? JSON.parse(savedAnswers) : {}
//         if (!parsedAnswers[11]) {
//           setAnswers((prev) => ({ ...prev, 11: getCurrentTime() }))
//         }
//         if (!parsedAnswers[20]) {
//           setAnswers((prev) => ({ ...prev, 20: getCurrentTime() }))
//         }
//       }
//     }
//   }, [currentStep])
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && studentName) {
      localStorage.setItem("math2StudentName", studentName)
      localStorage.setItem("math2Date", date)
      localStorage.setItem("math2Answers", JSON.stringify(answers))
      localStorage.setItem("math2CurrentStep", String(currentStep))
    }
  ...
  useEffect(() => {
    if (submitted && typeof window !== "undefined") {
      localStorage.removeItem("math2StudentName")
      localStorage.removeItem("math2Date")
      localStorage.removeItem("math2Answers")
      localStorage.removeItem("math2CurrentStep")
    }
  }, [submitted])

  // Ajouter un useEffect supplémentaire pour mettre à jour l'heure lorsque l'utilisateur passe à l'étape des questions
  useEffect(() => {
    if (currentStep === 1) {
      // Mettre à jour l'heure actuelle pour les questions d'horloge
      setAnswers((prev) => ({
        ...prev,
        11: getCurrentTime(),
        20: getCurrentTime(),
          ...
          <div key={question.id} className="bg-white p-6 rounded-lg shadow-md mb-6">
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                {/* Pour les questions d'horloge (11 et 20), afficher l'heure actuelle */}
                {(question.id === 11 || question.id === 20) && (
                  <div className="mt-4 mb-4">
                    ...
                    <input
                      ...
                      readOnly
                      className="mt-2 border-b-2 border-gray-400 h-10 focus:outline-none px-2 bg-gray-50"
                    />
                  </div>
                )}
