...
const Grade3ELA: React.FC = () => {
  ...
  const handleMatchingAnswerChange = (questionId: number, fromIndex: number, toIndex: number) => {

    const currentAnswers = Array.isArray(answers[questionId]) ? answers[questionId] as string[] : []
    const newAnswers = [...currentAnswers]

    newAnswers[fromIndex] = toIndex.toString()
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-6">
                  {question.columns?.map((column, columnIndex) => (
                    <div key={columnIndex} className="space-y-2">
                      <h4 className="font-bold text-gray-700 mb-2">{column.title}</h4>
                      {column.items.map((item, itemIndex) => (
                        <div key={itemIndex} className="p-2 border rounded-md bg-gray-50">
                          {item}
                        </div>
                      ))}
                    </div>
                ...
                <p className="text-sm text-gray-500 mt-4">
                  Note: For matching questions, please write your answers on paper or use the PDF version.
                </p>
              </div>
            </div>
          </div>
