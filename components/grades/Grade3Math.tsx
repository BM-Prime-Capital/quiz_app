...
import { grade3MathData, type Question } from "@/data/grade3/mathData"
import Link from "next/link"
import { ArrowLeft, CheckCircle } from "lucide-react"

const Grade3Math: React.FC = () => {
  const quizData = grade3MathData
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedName = localStorage.getItem("math3StudentName")
      const savedAnswers = localStorage.getItem("math3Answers")
      const savedStep = localStorage.getItem("math3CurrentStep")
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && studentName) {
      localStorage.setItem("math3StudentName", studentName)
      localStorage.setItem("math3Date", date)
      localStorage.setItem("math3Answers", JSON.stringify(answers))
      localStorage.setItem("math3CurrentStep", String(currentStep))
    }
  ...
  useEffect(() => {
    if (submitted && typeof window !== "undefined") {
      localStorage.removeItem("math3StudentName")
      localStorage.removeItem("math3Date")
      localStorage.removeItem("math3Answers")
      localStorage.removeItem("math3CurrentStep")
    }
  }, [submitted])
              ...
              <div className="flex-1">
                ...
                <div className="mt-4">
                  <select
                    ...
                    className="border-2 border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  >
